...
The backward pass of attention proceeds as follows. Let dO ∈ R
𝑁 ×𝑑 be the gradient of O with respect to
...
memory accesses translates to slow wall-clock time. Moreover, the required memory is 𝑂(𝑁
2
) due to having
to materialize S and P. Moreover, one has to save P ∈ R
𝑁 ×𝑁 for the backward pass to compute the gradients.
2.3 FlashAttention
...
FLOPs:
1. We do not have to rescale both terms of the output update by diag(ℓ
(2)
)
−1
:
...
(2)
.
We can instead maintain an “un-scaled” version of O(2) and keep around the statistics ℓ
(2)
:
O˜ (2) = diag(ℓ
(1)
...
(2)
.
Only at the every end of the loop do we scale the final O˜ (last) by diag(ℓ
(last)
)
−1
to get the right output.
2. We do not have to save both the max 𝑚
( 𝑗) and the sum of exponentials ℓ
( 𝑗)
for the backward pass. We
only need to store the logsumexp 𝐿
( 𝑗) = 𝑚
( 𝑗) + log(ℓ
( 𝑗)
).
...
and V1, . . . , V𝑇𝑐
, of size 𝐵𝑐 × 𝑑 each.
2: Divide the output O ∈ R
𝑁 ×𝑑
into 𝑇𝑟 blocks O𝑖
, . . . , O𝑇𝑟
of size 𝐵𝑟 × 𝑑 each, and divide the logsumexp 𝐿
into 𝑇𝑟 blocks 𝐿𝑖
, . . . , 𝐿𝑇𝑟
of size 𝐵𝑟 each.
3: for 1 ≤ 𝑖 ≤ 𝑇𝑟 do
4: Load Q𝑖 from HBM to on-chip SRAM.
5: On chip, initialize O
(0)
𝑖
= (0)𝐵𝑟×𝑑 ∈ R
𝐵𝑟×𝑑
...
the correct output O = softmax(QK>)V (with no approximation), using 𝑂(𝑁
2𝑑) FLOPs and requires 𝑂(𝑁)
...
and V1, . . . , V𝑇𝑐
, of size 𝐵𝑐 × 𝑑 each.
2: Divide O into 𝑇𝑟 blocks O𝑖
, . . . , O𝑇𝑟
of size 𝐵𝑟 × 𝑑 each, divide dO into 𝑇𝑟 blocks dO𝑖
, . . . , dO𝑇𝑟
of size
𝐵𝑟 × 𝑑 each, and divide 𝐿 into 𝑇𝑟 blocks 𝐿𝑖
, . . . , 𝐿𝑇𝑟
of size 𝐵𝑟 each.
3: Initialize dQ = (0)𝑁 ×𝑑 in HBM and divide it into 𝑇𝑟 blocks dQ1
, . . . , dQ𝑇𝑟
of size 𝐵𝑟 × 𝑑 each. Divide
dK, dV ∈ R
𝑁 ×𝑑
in to 𝑇𝑐 blocks dK1, . . . , dK𝑇𝑐
and dV1, . . . , dV𝑇𝑐
, of size 𝐵𝑐 × 𝑑 each.
4: Compute 𝐷 = rowsum(dO ◦ O) ∈ R
𝑑
(pointwise multiply), write 𝐷 to HBM and divide it into 𝑇𝑟 blocks
𝐷1, . . . , 𝐷𝑇𝑟
of size 𝐵𝑟 each.
5: for 1 ≤ 𝑗 ≤ 𝑇𝑐 do
6: Load K𝑗
...
, dQ𝑖
, 𝐿𝑖
, 𝐷𝑖 from HBM to on-chip SRAM.
10: On chip, compute S
( 𝑗)
𝑖
= Q𝑖K𝑇
𝑗
...
.
15: Load dQ𝑖
from HBM to SRAM, then on chip, update dQ𝑖 ← dQ𝑖 + dS( 𝑗)
𝑖 K𝑗 ∈ R
𝐵𝑟×𝑑
, and write back
to HBM.
16: On chip, compute dK𝑗 ← dK𝑗 + dS( 𝑗)
𝑖
>
Q𝑖 ∈ R
𝐵𝑐×𝑑
...
in Algorithm 2, where we need to load dQ𝑖
from HBM to SRAM, then on chip, update dQ𝑖 ← dQ𝑖 + dS( 𝑗)
𝑖 K𝑗
,
...
sequence lengths either 2k or 8k, FlashAttention-2 yields up to 1.3× speedup compared to FlashAttention and 2.8× speedup compared to a baseline without FlashAttention. FlashAttention-2
...
xformers (the “cutlass” implementation). FlashAttention-2 is around 1.3-1.5× faster than FlashAttention in Triton in the forward pass and around 2× faster in the backward pass. Compared to a standard
...
80GB SXM4. As shown in Table 1, FlashAttention-2 yields 2.8× speedup compared to a baseline without
FlashAttention and 1.3× speedup compared to FlashAttention, reaching up to 225 TFLOPs/s per
A100 GPU.
...
longer context for the same price as previously training a 8k context model, for the same number of tokens.
11
512 1k 2k 4k 8k 16k
Sequence length
50
...
[13] Markus N Rabe and Charles Staats. Self-attention does not need 𝑂(𝑛
2
